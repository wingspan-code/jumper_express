...
export const VerifyDiscordInteraction = async (publicKey, req, cb) => {
  const signature = req.headers.get("X-Signature-Ed25519");
  const timestamp = req.headers.get("X-Signature-Timestamp");
  const body = await req.bufferArray();
  console.log("Discord Request body: ", body);
  const isVerified = verifyKey(body, signature, timestamp, publicKey); 
  if (!isVerified) {
    console.log("Caught invalid Signature! >:)");
